...
def app(platform, device, force_cpu, theme="tools/themes/theme_blue.json", *, tasks):
	webui_config = deepcopy(load_configs(WEBUI_CONFIG))
	language_dict = load_configs(LANGUAGE)
	# Physical GPU selection belongs to Hub; native callbacks see the leased GPU as 0.
	for section in ("inference", "training"):
		webui_config[section]["device"] = list(device.values())
	with gr.Blocks(theme=gr.Theme.load(theme), title="MSST WebUI") as webui:
		gr.Markdown(value=f"""### Music-Source-Separation-Training-Inference-Webui""", visible=False)
		gr.Markdown(
			value=i18n(
		...
		gr.Markdown(value="""
		## MMST WebUI v{PACKAGE_VERSION}

		## [最佳实践文档](https://doc.weixin.qq.com/doc/w3_AXgA6AabAOECNrZpDgYQmSACwxc88?scode=ACkA3Ad4ABEJI09PtO)
		 """)

		gr.Markdown("GPU 0 是当前 Hub 租约映射的逻辑 cuda:0；物理 GPU 由模型池分配。CPU 模式沿用原参数。")

		with gr.Tabs():
			with gr.TabItem(label=i18n("MSST分离")):
				ui.msst(webui_config, device, force_cpu, tasks=tasks)
			with gr.TabItem(label=i18n("UVR分离")):
				ui.vr(webui_config, force_cpu, tasks=tasks)
			with gr.TabItem(label=i18n("预设流程")):
				ui.preset(webui_config, force_cpu, tasks=tasks)
			with gr.TabItem(label=i18n("合奏模式")):
				ui.ensemble(webui_config, force_cpu, tasks=tasks)
			with gr.TabItem(label=i18n("小工具")):
				ui.tools(webui_config, tasks=tasks)
			with gr.TabItem(label=i18n("安装模型")):
				ui.models(webui_config)
			with gr.TabItem(label=i18n("MSST训练")):
				ui.train(webui_config, device, tasks=tasks)
			...
			if os.environ.get("MSST_USE_WEBVIEW", "0") == "1":
				from .pages.weblog import weblog
				weblog()
