...
from webui.ensemble import ensemble_files, update_model_stem, add_to_ensemble_flow, reset_flow_func, reset_last_func, save_ensemble_preset_func, load_ensemble


def ensemble(webui_config, force_cpu_flag=False, *, tasks):
	inference_audio_func = partial(tasks.run_ui, 'ensemble.inference_audio_func')
	inference_folder_func = partial(tasks.run_ui, 'ensemble.inference_folder_func')
	stop_ensemble_func = partial(tasks.stop_ui, 'ensemble')
