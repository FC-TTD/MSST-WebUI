...
from webui.msst import update_selected_model, load_selected_model, save_model_config, reset_model_config, update_inference_settings

def msst(webui_config, device, force_cpu_flag=False, *, tasks):
	run_inference_single = partial(tasks.run_ui, 'msst.run_inference_single')
	run_multi_inference = partial(tasks.run_ui, 'msst.run_multi_inference')
	run_folder_batch_inference = partial(tasks.run_ui, 'msst.run_folder_batch_inference')
	stop_msst_inference = partial(tasks.stop_ui, 'msst')
	...
	with gr.Group(visible=False) as preview_group:
		gr.Markdown(i18n("### 结果预览"))
		with gr.Row():
			preview_state = gr.State([])
			preview_select = gr.Dropdown(label=i18n("选择文件"), interactive=True, scale=3)
		preview_player = gr.Audio(label=i18n("播放预览"), interactive=False)
	...
	preview_select.change(
		...
		inputs=[preview_state, preview_select],
